...
export const getUserAnalyticsWithChartService = async (
  ...
  const [{ chartData = [], totaluploadFilesForPeriod = 0, totalUsageForPeriod = 0 } = {}] = result;
  ...
  const storageMetrics = await StorageModel.getStorageMetrics(new Types.ObjectId(userId));
