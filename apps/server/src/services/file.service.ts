      ...
      try {
        ...
        if (_storageKey) {
          try {
            ...
            logger.info(`Rolled back S3 file: ${_storageKey}`);
          } catch (cleanupErr) {
            logger.error('Failed to cleanup S3 after upload error', cleanupErr);
          }
        }
        throw error;
      }
...
export const getAllFilesService = async (
  ...
  const [files, totalCount] = await Promise.all([
    FileModel.find(filterConditons).skip(skip).limit(pageSize).sort({ createdAt: -1 }),
...
export const downloadFilesService = async (userId: string, fileIds: string[]) => {
  const files = await FileModel.find({
    _id: { $in: fileIds },
  });
  if (!files.length) throw new NotFoundException('No files found');
...
async function handleMultipleFilesDownload(
  ...
  const zipFilename = `vaulta-${timestamp}.zip`;
